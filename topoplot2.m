function [] = topoplot2(nch, ijw, n_features, n_scal, cbtype)
% 
% % f_PlotEEG_BrainNetwork.m plots a brain functional network based on 4 EEG
% layouts of (31, 56, 61 and 21) channels and different options according
% the user needs.
% 
% USAGE.
...
%           these set of nodes: 31, 56, 61 and 21. In future releases, if
...
% % % % aij61 = importdata('aij61.txt');    aij21 = importdata('aij21.txt');
...
%     disp(nch);                          %Displaying number of nodes (activate it for testing purposes)
    ...
    return;
elseif nargin == 2                      %Previous + Matrix and the Scaling of Weights
...
elseif nargin == 5                      %Previous + Nodes' attributes and Node Scaling
    w_atribut = ijw(:, 3);              
...
% EasycapM11-61-Channel-Arrangement (used in BrainCap21). EasyCap Enterprises.
% For 21 + (ground) + (QRS) Montage (complete montage including ground and heart)
% 21 channels: 61 + 3 periferal channels Montage
% 61 channels Montage
% 56 channels Montage
% 31 channels Montage
...
elseif nchannels == 21 %21
    labels = labelsEEG{1};      %labels of a 21 montage
else
    warning('Type the correct number of channels: 31,56,61,21')
    error('Here are just 4 layouts for 31, 56, 61 and 21 channels in EaisyCap Enterprises');
end
...
for lk = 1 : size(ijw, 1)           %along all links
    ...
    xynodes(2, :) = r_nodepos2(ijw(lk,2), :); % for the second node of the link, hold the positions
%     line(xynodes(:, 1), xynodes(:, 2), 'LineWidth', w_atribut_new(lk), 'Color', RGBlinks(lk, :));
        line(xynodes(:, 1), xynodes(:, 2), 'LineWidth', ijw(lk, 3), 'Color', [.8 .8 .1]); % links de un solo color
                               ...
                               %for the two nodes of a link
    plot(xynodes(1, 1), xynodes(1, 2), 'o', 'MarkerSize', 5*pi.*n_atribut_new(ijw(lk,1)), ...
        'MarkerEdgeColor','k', 'MarkerFaceColor', [.8 .8 .1], 'LineWidth',1.5); % change node color into  RGBnodes(ijw(lk,1), :)
    text(xynodes(1, 1)-0.01, xynodes(1, 2)-0.01, labels{ijw(lk,1)}, 'fontsize', 15);

    plot(xynodes(2, 1), xynodes(2, 2), 'o', 'MarkerSize', 5*pi.*n_atribut_new(ijw(lk,2)+21), ...
        'MarkerEdgeColor','k', 'MarkerFaceColor',[.8 .8 .1], 'LineWidth',1.5); % change node color into RGBnodes(ijw(lk,2)+21, :)
    text(xynodes(2, 1)-0.01, xynodes(2, 2)-0.01, labels{ijw(lk,2)}, 'fontsize', 15);
end
...
elseif nchannels == 61
    labels = labelsEEG{3};      %labels of a 61 montage
elseif nchannels == 21
    labels = labelsEEG{4};      %labels of a 21 montage
else
    warning('Type the correct number of channels: 31,56,61,21')
    error('Here are just 4 layouts for 31, 56, 61 and 21 channels in EaisyCap Enterprises');
end
